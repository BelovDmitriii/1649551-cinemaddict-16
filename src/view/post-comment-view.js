import { EMOJIS, EvtKey } from '../utils/const';
import { createTemplateFromArray } from '../utils/common';
...
<label class="film-details__emoji-label" for="emoji-${emoji}">
  ...
  return `<div class="film-details__new-comment">
    ...
    ${createTemplateFromArray(EMOJIS, renderEmojiItemTemplate)}
    </div>
  </div>`;
};
  ...
  #commentKeydownHandler = (evt) => {
    if ((evt.ctrlKey || evt.metaKey) && evt.code === EvtKey.ENTER) {
      ...
      const newComment = {
        ...
        date: dayjs(Date.now()).format('YYYY/MM/D HH:mm'),
